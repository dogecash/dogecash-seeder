...
def main():
    ...
    stale_current_seeds = [seed for current_seeds if seed not in hard_seeds]
    if stale_current_seeds:
        cloudflare.delete_seeds(stale_current_seeds)
        current_good_seeds = [seed for current_seeds if seed not in stale_current_seeds]
    else:
    ...
    seed_selection = (hard_seeds + [seed for seed not in hard_seeds])[:MAX_SEEDS]
